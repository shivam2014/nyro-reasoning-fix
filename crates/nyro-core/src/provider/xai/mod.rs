...
impl VendorExtension for XaiVendor {
    fn scope(&self) -> VendorScope { VendorScope::Vendor { vendor_id: "xai" } }
...
#[async_trait]
impl ProviderAdapter for XaiVendor {
    fn vendor_id(&self) -> &'static str { "xai" }
    ...
    fn map_error(&self, status: u16, body: Value) -> GatewayError { openai_map_error("xai", status, body) }
}

inventory::submit! { VendorRegistration { make: || Box::new(XaiVendor) } }
inventory::submit! { ProviderAdapterRegistration { make: || Box::new(XaiVendor) } }
