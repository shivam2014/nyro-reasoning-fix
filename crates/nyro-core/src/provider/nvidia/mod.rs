...
impl VendorExtension for NvidiaVendor {
    fn scope(&self) -> VendorScope { VendorScope::Vendor { vendor_id: "nvidia" } }
...
#[async_trait]
impl ProviderAdapter for NvidiaVendor {
    fn vendor_id(&self) -> &'static str { "nvidia" }
    ...
    fn map_error(&self, status: u16, body: Value) -> GatewayError { openai_map_error("nvidia", status, body) }
}

inventory::submit! { VendorRegistration { make: || Box::new(NvidiaVendor) } }
inventory::submit! { ProviderAdapterRegistration { make: || Box::new(NvidiaVendor) } }
